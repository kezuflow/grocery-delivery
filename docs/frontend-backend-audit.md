...
| Marketplace             | Catalog, search, categories, availability, server prices, quantities, cart persistence             | unverified | Local convergence implemented; staging evidence remains pending AUD-001 |
...
- **Evidence:** the route already loads `loadMarketplace()` and uses the typed catalog/cart client;
  before this increment it rendered inside the dashboard-oriented `AppShell`, so the Figma `21_Home`
  phone pattern and Hatbazar desktop merchandising hierarchy were not converged.
...
- **Local evidence:** web lint, typecheck, and 57 focused tests pass. Staging visual, accessibility,
  latency, and cross-role evidence is still required, so classification remains `unverified`.
- **Severity/dependency:** P2 product gap with no backend dependency; staging browser evidence is the
  resume point before AUD-001 can classify the marketplace as matched.
