...
Frontend history is archived separately so future models do not mistake old layer-oriented plans
for active scope. The platform/API launch gate in Slice 022 remains independently blocked until its
documented staging evidence exists.
...
Frontend architecture, accessibility, styling, imagery, and per-slice verification rules live in
[`docs/frontend-standards.md`](frontend-standards.md). Deployment and rollback guidance lives in
[`docs/runbooks/frontend-release-handoff.md`](runbooks/frontend-release-handoff.md). The former
layer-oriented frontend ledger is archived at
[`docs/archive/frontend-implementation-backlog-legacy.md`](archive/frontend-implementation-backlog-legacy.md)
for historical context only. Do not use its FE identifiers or deferred-testing instructions to plan
new work; current scope and status belong to the unified `AUD-*` and `VS-*` process above.
