...
- **Responsive marketplace header alignment:** the phone and tablet header now uses the same white,
  neutral visual language as the desktop shell instead of a separate green inverse treatment. The
  responsive chrome is one horizontal row with the account control, visible `freshmarkets` name,
  search, and cart; its content spans the available width, and the previous back arrow that linked
  `/shop` to itself is replaced by the shared account-menu control. Live local browser review covers
  ...
  focused component lint also passes. The repository-wide `pnpm check`, web package lint,
  typecheck, and unit suite are currently blocked by unrelated active catalog `categoryIds` changes
  and their stale imports/tests, while the header slice adds no request, persistence, latency,
  runtime dependency, or observability boundary.
...
- **Produce catalog assets:** `docs/marketplace-produce-master-list.md` records the complete 142
  vegetable/root-crop and 83 fruit entries from the current PSA OpenSTAT crop tables, plus the
  requested Japanese cucumber retail variant. All 226 transparent 512×512 WebP product assets are
  generated under `apps/web/public/marketplace/produce/`, with aggregate products and named
  varieties such as Banana, Cavendish, Lakatan, Latundan, and Saba kept as separate files. This
  local asset slice does not publish new SKUs, change pricing, or add runtime requests.
...
- **Implementation:** added marketplace semantic tokens, introduced `MarketplacePageShell`, migrated customer account/cart/checkout/orders/saved/support/subscription routes off the generic account shell, linked product cards to detail routes, aligned mobile navigation to Shop/Aisles/Orders/Account, kept delivery context visible in the mobile shell, added sticky mobile product purchase actions, rounded the actual product image canvases instead of only their outer frames, reduced fallback promotion height to preserve product discovery, and redesigned cart, checkout, order history/detail, account navigation, and account child form surfaces while preserving their existing feature contracts.
- **Local evidence:** `/shop` verified at 1440 by 900 and 390 by 844; `/shop/apple` verified at 390 by 844 with persistent delivery context, the expected mobile navigation model, a reachable sticky purchase action, no horizontal overflow, and no browser console errors. API served locally on `127.0.0.1:8788`; the web server served locally on `localhost:3000`. Unauthenticated `/account` correctly redirected to `/forbidden`. The current browser session cannot exercise protected authenticated pages; customer fixture E2E scenarios cover cart, checkout, payment retry, reorder, and order tracking.
